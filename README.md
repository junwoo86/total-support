...
| `GET`    | `/api/grant/postings` | 공고 목록 (서버 페이지네이션·필터·D-Day, 적합도 정렬) |
| `GET`    | `/api/grant/postings/counts` | 검토 상태별 카운트 — `{UNREVIEWED, NEEDS_REVIEW, IN_PROGRESS, EXCLUDED}` |
...
- **검토 상태별 카운트 엔드포인트** `/postings/counts` — `GROUP BY review_status`
  단일 쿼리, status 외 필터(domain/q/hide_expired 등)는 list 와 동일하게 적용
- **다중값 필터 + 적합도 버킷** — `status`, `suitability`, `site`, `domain`,
  `relevance_bucket(high|mid_high|mid|low)` 모두 CSV 다중값을 받아 `IN` / `OR` 결합
- **229 테스트 통과** (단위 + 라우터 통합 + 서비스 직접 + Enum drift 가드 + evaluator mock
  + counts/bucket/CSV 파서)

### 프론트엔드
- React 18 SPA (UMD + Babel Standalone — 빌드 단계 없음)
- 5탭: 신규 미검토 / 검토 상태별 확인 / 분야·키워드 관리 / 사이트별 수집 상태 / 시스템 로그
- **공고 목록 7 컬럼**: 지원 사업명 / 분야 / **적합도(AI 0~100%)** / **적합 사유**
  / 상태 / 접수 기간·D-Day / 내부 검토 — 분석 실패 행은 빨간 배지로 최상단 고정
- **서버 페이지네이션** — 200건씩 누적 로드 (`＋ 더 불러오기`). 백엔드의
  적합도 ORDER BY 가 보장하는 정렬 순서를 깨지 않음 (200건 cap 묻힘 없음)
- **다중 선택 필터 칩** (적합도 / 분야 / 상태) — 같은 그룹 안에서 OR, 그룹 간 AND.
  서버는 CSV 파라미터로 받음 (`status=NEEDS_REVIEW,IN_PROGRESS`)
- **적합도 4-bucket 필터** — 상(80%↑) / 중상(60%↑) / 중(40%↑) / 하(40%↓)
- **D-Day 정렬 토글** (신규 미검토 탭) — 현재 화면 30건을 마감 가까운 순으로
  클라이언트 재정렬. "적합도 상 + 마감 임박" 워크플로우에 유용
- **검토 상태별 카운트** — StatusTab 의 탭 옆 합산 숫자 + 칩 옆 개별 숫자
  (`/postings/counts` 엔드포인트, PATCH 시 즉시 갱신)
